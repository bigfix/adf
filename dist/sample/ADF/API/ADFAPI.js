...
tem.relevance.evaluate = function(relevance, callbackObj, customSynchronousErrorHandler) {
	...
	if(callbackObj !== undefined && callbackObj !== null) {
		...
		if (tem.config.isDebug()) {
			tem.logging.debug('Relevance call (START): ' + relevance.substring(0, 90) + "...");
		}
...
tem.file.upload = {};
	...
	/**
	 * @namespace <b>It's recommended that you use {@link tem.file.FileSystem} (where ActiveX use is available) rather than scratchPad functions</b>
...
tem.content = {};
	...
	/**
	 * <b>Not available in Web Reports</b> Used to bring up the take action dialog as if the user had clicked the take 
	 ...
	 * tem.content.takeFixletAction( fixletID, 0xffffffff & tem.evalRel('id of bes site whose (name of it is "ActionSite")'), contentID )</pre>
	 ...
	 */
	tem.content.takeFixletAction = function(actionID, siteID, contentID, parameters, callback) {
		return takeFixletActionHook(actionID, siteID, contentID, parameters, callback);
	};
...
(function() {
	...
	tem.context.getBaseDirectory = function() {
		...
		try {
			...
			if(baseDir!==null){
				// ie8+ returns file:... url
				if(baseDir.slice(0,5) == "file:") {
					baseDir = unescape(baseDir).slice(8);
					var dirs = baseDir.split("/");
					dirs.pop();
					baseDir = dirs.join("\\");
					
				} else if(  baseDir.lastIndexOf("\\") === baseDir.length-1 ){
...
tem.model=  {};
	 ...
	 * @property {String or tem.model.XMLWrapper} ActionScript
	 ...
	 * @param {String or tem.model.XMLWrapper} actionScript
	 * 
	...
	tem.model.SingleAction = function(title, relevance, actionScript) {
		...
		this._addTypeMapProperties({Settings:"ActionSettings", SettingsLocks:"XMLWrapper", IsUrgent:"Boolean"});
	 ...
	 * @property {String} Title
	 * @property {String} Relevance
	 * @property {String or tem.model.XMLWrapper} PreGroupActionScript
	 * @property {Array[tem.model.MemberAction]} MemberAction
	 * @property {String or tem.model.XMLWrapper} PostGroupActionScript
	 * @property {tem.model.ActionSettings} Settings
	 * @property {tem.model.XMLWrapper} SettingsLocks
	 * 
	 * @class
